package auth

import (
	"log"

	"net/http"

	"github.com/absagar/go-bcrypt"
	"github.com/gin-gonic/gin"

	"go-backend/api/accessToken"
	"go-backend/dao"
)

type OnboardUser struct {
	Email     string `json:"email"`
	FirstName string `json:"first_name"`
	LastName  string `json:"last_name"`
	Password  string `json:"password"`
	IsAdmin   bool   `json:"is_admin"`
}

type OnboardBody struct {
	User OnboardUser `json:"user"`
}

func Onboard(c *gin.Context) {
	var request OnboardBody

	if err := c.BindJSON(&request); err != nil {
		log.Println(err)
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "Request body is not of correct format",
		})
		return
	}

	adminId, ok := c.Get("UserID")
	if !ok {
		log.Println("Unable to get user id")
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "Something went wrong",
		})
		return
	}

	admin, err := dao.FindUserById(uint(adminId.(float64)))
	if err != nil {
		log.Println(err)
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "Unable to find admin user",
		})
		return
	}

	passwordHash, err := bcrypt.Hash(request.User.Password)
	if err != nil {
		log.Println(err)
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "Something went wrong",
		})
		return
	}

	user, err := dao.CreateEmployee(request.User.Email, request.User.FirstName, request.User.LastName, passwordHash, request.User.IsAdmin, admin.CompanyID)
	if err != nil {
		log.Println(err)
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "Unable to create employee",
		})
		return
	}

	token, err := accessToken.GenerateToken(user)
	if err != nil {
		log.Println(err)
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "Unable to generate token",
		})
		return
	}

	c.SetCookie("token", token, 86400, "/", "localhost", false, true)
	c.JSON(http.StatusOK, gin.H{
		"message":  "Successfully registered a new account",
		"is_admin": user.IsAdmin,
		"user_id":  user.ID,
	})
}
